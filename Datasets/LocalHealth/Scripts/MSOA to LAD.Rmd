...
Now we have our reduced lookup table we can join it to the local health data to allow us to group it. Note that this includes MSOA and LAD data for Wales where there is no Local Health data.
```{r}
LocalHealthMSOAdata2 <-LocalHealthMSOAdata %>% rename(MSOA11CD=AreaCode)

joined<-left_join(LocalHealthMSOAdata2,LookupTable, by='MSOA11CD')
```
