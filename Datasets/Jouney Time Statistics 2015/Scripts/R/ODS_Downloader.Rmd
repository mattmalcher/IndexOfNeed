...
for(i in 1:length(ttd)){
  write.csv(x = ttd[[i]],
            file = paste0(prodat,names(ttd)[i],'.csv'))
}
```
...
for(i in 1:length(ttd_meta)){
  write.csv(x = ttd_meta[[i]],
            file = paste0(prodat,names(ttd_meta)[i],'_metadata.csv'))
}
```
...
# join all of the data frames in the list ttd by their second columns.
merged_ttd <- Reduce(function(...) merge(..., by=2, all.x=TRUE), ttd)
